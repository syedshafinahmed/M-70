    ...
    <div className="relative bg-[#111] border border-white/10 rounded-3xl p-5 shadow-md animate-pulse">
      ...
      <div className="flex gap-1 mt-3">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-4 w-4 rounded-full bg-orange-500/30" />
        ))}
      </div>
      ...
      <div className="mt-3 flex justify-between items-center">
        <div className="flex items-center gap-1">
          <AiFillLike size={16} className="text-orange-500/30" />
          <div className="h-3 w-6 bg-orange-500/30 rounded" />
        </div>
